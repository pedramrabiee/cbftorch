...
def make_circle_barrier_functional(center, radius):
    center = vectorize_tensors(center)
    return (
        lambda x: torch.norm((vectorize_tensors(x)[..., :2] - center.to(x.device)), p=2, dim=-1)
                  / radius
                  - 1
    )
...
def make_norm_rectangular_barrier_functional(center, size, rotation=0.0, p=20):
    size, center = vectorize_tensors(size), vectorize_tensors(center)
    return (
        lambda x: torch.norm(
            (
                    rotate_tensors(
                        points=vectorize_tensors(x), center=center.to(x.device), angle_rad=-rotation
                    )
                    - center.to(x.device)
            )
            / size.to(x.device),
...
def make_affine_rectangular_barrier_functional(center, size, rotation=0.0, smooth=False, rho=40):
    ...
    A = torch.tensor([[1, 0], [-1, 0], [0, 1], [0, -1]], dtype=DEFAULT_DTYPE)
    # Rotate the normals
    b = torch.tensor(
        [center[0] + size[0], -center[0] + size[0], center[1] + size[1], -center[1] + size[1]],
        dtype=DEFAULT_DTYPE,
    ).unsqueeze(0)

    def affine_rectangle(x):
        rotate_x = rotate_tensors(
            points=vectorize_tensors(x[..., :2]), center=center.to(x.device), angle_rad=-rotation
        )
...
def make_affine_rectangular_boundary_functional(center, size, rotation=0.0, smooth=False, rho=40):
    return lambda x: -make_affine_rectangular_barrier_functional(
        center, size, rotation, smooth, rho
    )(x)
...
def make_box_barrier_functionals(bounds, idx):
    ...
    return [
        lambda x: vectorize_tensors(x)[..., idx] - torch.tensor(lb),
        lambda x: torch.tensor(ub) - vectorize_tensors(x)[..., idx],
    ]


def make_ellipse_barrier_functional(center, A):
    center = vectorize_tensors(tensify(center))
    A = tensify(A)
    return lambda x: 1 - torch.einsum(
        "bi,ij,bj->b", vectorize_tensors(x) - center, A, vectorize_tensors(x) - center
    ).unsqueeze(-1)
...
def tensify(obj):
    if isinstance(obj, torch.Tensor):
        return obj.to(DEFAULT_DTYPE)
    if isinstance(obj, np.ndarray):
        return torch.from_numpy(obj).to(DEFAULT_DTYPE)
    if isinstance(obj, (list, tuple)):
        return torch.tensor(obj, dtype=DEFAULT_DTYPE)
...
def lie_deriv_from_values(func_deriv, field_val):
    ...
    if field_val.ndim == 2:
        return torch.cat(
            [torch.einsum("bn,bn->b", fderiv, field_val).unsqueeze(1) for fderiv in func_deriv],
            dim=1,
        )
...
def rotate_tensors(points, center, angle_rad):
    ...
    rotated_xy = (
            torch.matmul(points[..., :2] - center[..., :2], rotation_matrix.t()) + center[..., :2]
    )
...
def get_trajs_from_action_func(x0, dynamics, action_func, timestep, sim_time, method="euler"):
    return odeint(
        func=lambda t, y: dynamics.rhs(y, action_func(y)),
        y0=x0,
...
def get_trajs_from_action_func_zoh(
        x0, dynamics, action_func, timestep, sim_time, intermediate_steps, method="dopri5"
):
    # Get dimensions
    ...
    trajs = torch.zeros((num_steps, batch_size, state_dim), device=x0.device, dtype=DEFAULT_DTYPE)
    trajs[0] = x0

    # Create intermediate time points for better integration
    t_local = torch.linspace(0, timestep, intermediate_steps, device=x0.device, dtype=DEFAULT_DTYPE)
    ...
    for i in range(num_steps - 1):
        ...
        next_states = odeint(
            lambda t, x: dynamics.rhs(x, current_controls), trajs[i], t_local, method=method
        )[-1]
...
def get_trajs_from_batched_action_func(
        x0, dynamics, action_funcs, timestep, sim_time, method="euler"
):
    action_num = len(action_funcs)
    return odeint(
        func=lambda t, y: torch.cat(
            [
                dynamics.rhs(yy.squeeze(0), action(yy.squeeze(0)))
...
class SVM:
    def __init__(self, cfg):
        ...
        self.kernel_fn = self._kernel_factory(
            cfg.kernel_type,
            sigma=cfg.sigma if cfg.kernel_type == "rbf" and hasattr(cfg, "sigma") else None,
            degree=(
                cfg.degree if cfg.kernel_type == "polynomial" and hasattr(cfg, "degree") else None
            ),
            coef=cfg.coef if cfg.kernel_type == "polynomial" and hasattr(cfg, "coef") else None,
        )
    ...
    def _kernel_factory(self, func_name, **kwargs):
        ...
        def rbf(x1, x2, sigma=1.0, degree=None, coef=None):
            ...
            pairwise_sq_dists = (
                    x1_norm_squared.unsqueeze(1)
                    + x2_norm_squared.unsqueeze(0)
                    - 2 * torch.einsum("ik,jk->ij", x1, x2)
            )
    ...
    def boundary_func(self, X, y, sv, sv_y, lambdas, b):
        return lambda x: (
                torch.einsum("ik,ik,ni->nk", lambdas, sv_y, self.kernel_fn(x, sv)).squeeze(-1) + b
        ).unsqueeze(-1)
    ...
    def fit(self, X, y, lambdas=None):
        ...
        c = -torch.ones(n_samples, dtype=DEFAULT_DTYPE)

        G_lower = -torch.eye(n_samples, dtype=DEFAULT_DTYPE)
        h_lower = torch.zeros(n_samples, dtype=DEFAULT_DTYPE)

        G_upper = torch.eye(n_samples, dtype=DEFAULT_DTYPE)
        ...
        A = y.t()
        b = torch.zeros(1, dtype=DEFAULT_DTYPE)
        ...
        solution = solvers.qp(
            matrix(Q_np), matrix(c_np), matrix(G_np), matrix(h_np), matrix(A_np), matrix(b_np)
        )
        lambdas = torch.tensor(np.array(solution["x"]), dtype=DEFAULT_DTYPE)
        ...
        b = (
                torch.sum(
                    sv_y - torch.sum(self.kernel_fn(sv, sv) * lambdas * sv_y, dim=-1).unsqueeze(-1),
                    dim=0,
                )
                / sv.shape[0]
        )
