...
from math import pi
from hocbf_composition.examples.single_integrator.single_integrator_dynamics import SIDynamics
...
from hocbf_composition.examples.single_integrator.map_config import map_config
from hocbf_composition.examples.single_integrator.single_integrator_desired_control import desired_control
...
dynamics = SIDynamics(state_dim=2, action_dim=2)
...
# Initial Conditions
x0 = torch.tensor([-1.0, -8.5]).repeat(goal_pos.shape[0], 1)
timestep = 0.01
sim_time = 12.0
...
# Plot actions
axs[1].plot(time, actions[0][:, 0], label=r'$u_1$', color='black')
axs[1].plot(time, des_ctrls[0][:, 0], color='red', linestyle='--', label=r'$u_{{\rm d}_1}$')
axs[1].legend(loc='upper right', ncol=2, frameon=False, fontsize=14)
axs[1].set_ylabel(r'$u_1$', fontsize=16)

axs[2].plot(time, actions[0][:, 1], label=r'$u_2$', color='black')
axs[2].plot(time, des_ctrls[0][:, 1],  color='red', linestyle='--', label=r'$u_{{\rm d}_2}$')
axs[2].legend(loc='upper right', ncol=2, frameon=False, fontsize=14)
axs[2].set_ylabel(r'$u_2$', fontsize=16)
