...
from hocbf_composition.examples.bicycle.map_config import map_config
from hocbf_composition.examples.bicycle.bicycle_desired_control import desired_control
...
dynamics = BicycleDynamics(state_dim=4, action_dim=2, params=AD(l=1))
...
safety_filter.assign_desired_control(
    desired_control=lambda x: vectorize_tensors(partial(desired_control, goal_pos=goal_pos, params=dynamics.params, **control_gains)(x))
)
...
for i, traj in enumerate(trajs):
    des_ctrl = lambda x: vectorize_tensors(
            partial(desired_control, goal_pos=goal_pos[i].repeat(x.shape[0], 1), params=dynamics.params, **control_gains)(x))
    safety_filter.assign_desired_control(
